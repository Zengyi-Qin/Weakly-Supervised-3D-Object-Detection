...
class Reader(object):
    ...
    def __init__(self, kitti_dir):
        image = os.path.join(kitti_dir, 'image_2')
        label = os.path.join(kitti_dir, 'label_2')
        calib = os.path.join(kitti_dir, 'calib')
        lidar = os.path.join(kitti_dir, 'velodyne')
        sphere = os.path.join(kitti_dir, 'sphere')
        planes = os.path.join(kitti_dir, 'planes')
        self.indices = []
        self.data = {}
        for label_file in os.listdir(label):
            ...
            data['sphere_path'] = os.path.join(sphere, index + '.npy')
            self.indices.append(index)
            calib_path = os.path.join(calib, index + '.txt')
            plane_path = os.path.join(planes, index + '.txt')
            with open(plane_path) as plane_file:
                plane = plane_file.readlines()[-1]
                plane = plane.strip().split(' ')
                data['plane'] = [float(coef) for coef in plane]
            with open(calib_path) as calib_file:
                lines = calib_file.readlines()
                P2 = np.reshape(lines[2].strip().split(' ')[1:], 
                         (3, 4)).astype(np.float32)
                
                Tr = np.zeros((4, 4), dtype=np.float32)
                Tr[3, 3] = 1.0
                Tr[:3, :] = np.reshape(lines[5].strip().split(' ')[1:], 
                                (3, 4)).astype(np.float32)
                
                R0 = np.zeros((4, 4), dtype=np.float32)
                R0[3, 3] = 1.0
                R0[:3, :3] = np.reshape(lines[4].strip().split(' ')[1:], 
